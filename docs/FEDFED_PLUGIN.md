...
在不改变 FedAvg 主流程的前提下，插件增加了一条低维投影通道：

- 客户端从中间特征 `h` 中投影出低维 prototype 表示 `z_s`
- 按类别统计本地 prototype
- 对 prototype 做裁剪和加噪
- 上传类原型到服务器
...
当前实现不再把 `z_s` / `z_r` 解释为一套严格的“共享特征/残差特征分离”。

更准确的表述是：

- `h` 仍然负责主任务分类
- `z_s` 是从 `h` 投影出的低维 prototype 蒸馏子空间
- prototype loss 作为额外正则项，抑制客户端间表示漂移

因此，本方法当前更适合定义为：

- 低维投影 + 类原型蒸馏插件
- 而不是严格意义上的 disentanglement 模型
