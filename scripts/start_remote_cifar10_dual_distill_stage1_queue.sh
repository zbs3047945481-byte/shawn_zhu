...
PREFIX="cifar10_dual_distill_stage1_$(date -u +%Y%m%dT%H%M%SZ)"
ROUND_NUM=100
BATCH_SIZE=256
...
QUEUE_FILE="${TMP_DIR}/dual_distill_stage1_queue.ps1"
LAUNCHER_FILE="${TMP_DIR}/dual_distill_stage1_queue_launcher.ps1"
REMOTE_QUEUE_PATH="${REMOTE_Codex_DIR}/dual_distill_stage1_queue_${PREFIX}.ps1"
REMOTE_LAUNCHER_PATH="${REMOTE_Codex_DIR}/dual_distill_stage1_queue_launcher_${PREFIX}.ps1"
...
function Run-One(\$name, \$pluginName, \$extraArgs) {
  ...
  [ordered]@{
    ...
    mode = 'dual_distill_stage1_queue'
    arguments = \$trainArgs
...
try {
  ...
  Write-QueueStatus 'starting' '' 'CIFAR-10 dual-distill stage-1 grid starting.'
  Run-One 'fedavg' 'none' @()
  Run-One 'anchor_only' 'fedfed_prototype' (\$baseDualArgs + @('--fedfed_enable_distill', 'false'))
  ...
  Write-QueueStatus 'succeeded' '' 'CIFAR-10 dual-distill stage-1 grid finished.'
} catch {
...
echo "Offline dual-distill stage-1 queue submitted: ${PREFIX}"
