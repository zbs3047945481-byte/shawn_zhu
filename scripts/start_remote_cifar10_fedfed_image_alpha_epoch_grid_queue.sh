...
PREFIX="cifar10_fedfed_image_grid_$(date -u +%Y%m%dT%H%M%SZ)"
ROUND_NUM=100
BATCH_SIZE=256
...
QUEUE_FILE="${TMP_DIR}/fedfed_image_grid_queue.ps1"
LAUNCHER_FILE="${TMP_DIR}/fedfed_image_grid_queue_launcher.ps1"
REMOTE_QUEUE_PATH="${REMOTE_Codex_DIR}/fedfed_image_grid_queue_${PREFIX}.ps1"
REMOTE_LAUNCHER_PATH="${REMOTE_Codex_DIR}/fedfed_image_grid_queue_launcher_${PREFIX}.ps1"
...
function Run-One(\$alpha, \$epoch, \$name, \$pluginName) {
  \$alphaTag = \$alpha.Replace('.', 'p')
  \$runId = "\${prefix}_a\${alphaTag}_ep\${epoch}_\${name}"
  ...
  [ordered]@{
    ...
    mode = 'fedfed_image_alpha_epoch_grid_queue'
    arguments = \$trainArgs
  ...
  Write-QueueStatus 'running' \$runId "Training alpha=\$alpha epoch=\$epoch method=\$name."
...
try {
  ...
  Write-QueueStatus 'starting' '' 'FedAvg vs FedFedImage alpha/epoch grid starting.'
  foreach (\$alpha in @('0.1', '0.3')) {
    foreach (\$epoch in @('1', '5')) {
      Run-One \$alpha \$epoch 'fedavg' 'none'
      Run-One \$alpha \$epoch 'fedfed_image' 'fedfed_image'
    }
  }
  Write-QueueStatus 'succeeded' '' 'FedAvg vs FedFedImage alpha/epoch grid finished.'
} catch {
...
echo "FedFed image alpha/epoch grid queue submitted: ${PREFIX}"
