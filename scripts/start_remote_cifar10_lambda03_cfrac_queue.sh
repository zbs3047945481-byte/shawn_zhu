...
PREFIX="cifar10_lambda03_cfrac_$(date -u +%Y%m%dT%H%M%SZ)"
WAIT_PREFIX=""
ROUND_NUM=100
BATCH_SIZE=256
...
QUEUE_FILE="${TMP_DIR}/lambda03_cfrac_queue.ps1"
LAUNCHER_FILE="${TMP_DIR}/lambda03_cfrac_queue_launcher.ps1"
REMOTE_QUEUE_PATH="${REMOTE_Codex_DIR}/lambda03_cfrac_queue_${PREFIX}.ps1"
REMOTE_LAUNCHER_PATH="${REMOTE_Codex_DIR}/lambda03_cfrac_queue_launcher_${PREFIX}.ps1"
...
function Wait-For-Prefix(\$targetPrefix) {
  if ([string]::IsNullOrWhiteSpace(\$targetPrefix)) {
    return
  }
  \$targetStatusPath = Join-Path \$runsRoot ("\$targetPrefix" + '_queue_status.json')
  Write-QueueStatus 'waiting' '' ("Waiting for " + \$targetPrefix)
  while (\$true) {
    if (Test-Path \$targetStatusPath) {
      ...
      if (\$status.status -in @('succeeded', 'failed', 'stopped')) {
        if (\$status.status -ne 'succeeded') {
          throw ("Previous queue {0} ended with status {1}" -f \$targetPrefix, \$status.status)
        }
        return
      }
    }
...
function Run-One(\$name, \$cFraction, \$warmupRounds) {
  \$runId = "\${prefix}_\${name}_cf\${cFraction}_a03_ep5"
  ...
  \$trainArgs = @(
    ...
    '--fedfed_distill_warmup_rounds', \$warmupRounds,
...
try {
  ...
  Wait-For-Prefix \$waitPrefix
  Write-QueueStatus 'starting' '' 'CIFAR-10 lambda=0.3 c_fraction/warmup grid starting.'
  Run-One 'lambda03_warmup5' '0.2' '5'
  Run-One 'lambda03_warmup10' '0.2' '10'
  Run-One 'lambda03_warmup5' '0.4' '5'
  Run-One 'lambda03_warmup10' '0.4' '10'
  Write-QueueStatus 'succeeded' '' 'CIFAR-10 lambda=0.3 c_fraction/warmup grid finished.'
} catch {
...
echo "Offline lambda=0.3 c_fraction queue submitted: ${PREFIX}"
