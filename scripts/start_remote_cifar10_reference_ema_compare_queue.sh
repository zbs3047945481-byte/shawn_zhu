...
PREFIX="cifar10_refproto_ema_compare_$(date -u +%Y%m%dT%H%M%SZ)"
ROUND_NUM=100
BATCH_SIZE=256
...
QUEUE_FILE="${TMP_DIR}/reference_ema_compare_queue.ps1"
LAUNCHER_FILE="${TMP_DIR}/reference_ema_compare_queue_launcher.ps1"
REMOTE_QUEUE_PATH="${REMOTE_Codex_DIR}/reference_ema_compare_queue_${PREFIX}.ps1"
REMOTE_LAUNCHER_PATH="${REMOTE_Codex_DIR}/reference_ema_compare_queue_launcher_${PREFIX}.ps1"
...
function Run-One(\$name, \$pluginName, \$extraArgs) {
  ...
  [ordered]@{
    ...
    mode = 'reference_ema_compare_queue'
    arguments = \$trainArgs
...
\$baseDualArgs = @(
  ...
  '--fedfed_lambda_distill', '0.1',
  '--fedfed_distill_warmup_rounds', '5'
)

try {
  New-Item -ItemType Directory -Force -Path \$runsRoot | Out-Null
  Write-QueueStatus 'starting' '' 'CIFAR-10 reference-prototype EMA comparison starting.'
  Run-One 'fedavg' 'none' @()
  Run-One 'lambda01_warmup5_noema' 'fedfed_prototype' (\$baseDualArgs + @('--fedfed_prototype_momentum', '0'))
  Run-One 'lambda01_warmup5_ema08' 'fedfed_prototype' (\$baseDualArgs + @('--fedfed_prototype_momentum', '0.8'))
  Write-QueueStatus 'succeeded' '' 'CIFAR-10 reference-prototype EMA comparison finished.'
} catch {
...
echo "Offline reference EMA comparison queue submitted: ${PREFIX}"
