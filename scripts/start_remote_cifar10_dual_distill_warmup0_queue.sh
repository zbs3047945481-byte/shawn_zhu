...
PREFIX="cifar10_dual_distill_warmup0_$(date -u +%Y%m%dT%H%M%SZ)"
ROUND_NUM=100
BATCH_SIZE=256
...
QUEUE_FILE="${TMP_DIR}/dual_distill_warmup0_queue.ps1"
LAUNCHER_FILE="${TMP_DIR}/dual_distill_warmup0_queue_launcher.ps1"
REMOTE_QUEUE_PATH="${REMOTE_Codex_DIR}/dual_distill_warmup0_queue_${PREFIX}.ps1"
REMOTE_LAUNCHER_PATH="${REMOTE_Codex_DIR}/dual_distill_warmup0_queue_launcher_${PREFIX}.ps1"
...
function Run-One(\$name, \$extraArgs) {
  ...
  \$baseArgs = @(
    ...
    '--fedfed_distill_warmup_rounds', '0'
  )
  \$trainArgs = \$baseArgs + \$extraArgs
  ...
  [ordered]@{
    ...
    mode = 'dual_distill_warmup0_queue'
    arguments = \$trainArgs
...
try {
  ...
  Write-QueueStatus 'starting' '' 'CIFAR-10 dual-distill warmup0 grid starting.'
  Run-One 'lambda01_warmup0' @('--fedfed_lambda_distill', '0.1')
  Run-One 'lambda03_warmup0' @('--fedfed_lambda_distill', '0.3')
  Write-QueueStatus 'succeeded' '' 'CIFAR-10 dual-distill warmup0 grid finished.'
} catch {
...
echo "Offline dual-distill warmup0 queue submitted: ${PREFIX}"
