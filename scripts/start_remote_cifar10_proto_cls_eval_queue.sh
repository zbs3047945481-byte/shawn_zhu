...
PREFIX="cifar10_proto_cls_eval_$(date -u +%Y%m%dT%H%M%SZ)"
ROUND_NUM=100
BATCH_SIZE=256
...
QUEUE_FILE="${TMP_DIR}/proto_cls_eval_queue.ps1"
LAUNCHER_FILE="${TMP_DIR}/proto_cls_eval_queue_launcher.ps1"
REMOTE_QUEUE_PATH="${REMOTE_Codex_DIR}/proto_cls_eval_queue_${PREFIX}.ps1"
REMOTE_LAUNCHER_PATH="${REMOTE_Codex_DIR}/proto_cls_eval_queue_launcher_${PREFIX}.ps1"
...
try {
  ...
  Write-QueueStatus 'starting' '' 'CIFAR-10 proto-cls evaluation queue starting.'
  foreach (\$alpha in @('0.1', '0.3')) {
    foreach (\$epoch in @('1', '5')) {
      Run-One 'fedavg' \$alpha \$epoch 'none' @()
      Run-One 'warmup10_no_anchor' \$alpha \$epoch 'fedfed_prototype' @('--fedfed_distill_warmup_rounds', '10', '--fedfed_enable_anchor', 'false')
      Run-One 'warmup10_no_anchor_proto_cls' \$alpha \$epoch 'fedfed_prototype' @('--fedfed_distill_warmup_rounds', '10', '--fedfed_enable_anchor', 'false', '--fedfed_enable_proto_cls', 'true', '--fedfed_lambda_proto_cls', '0.1')
    }
  }
  Write-QueueStatus 'succeeded' '' 'CIFAR-10 proto-cls evaluation queue finished.'
} catch {
...
echo "Offline proto-cls evaluation queue submitted: ${PREFIX}"
