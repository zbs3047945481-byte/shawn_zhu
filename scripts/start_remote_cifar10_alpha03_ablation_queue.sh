...
QUEUE_FILE="${TMP_DIR}/offline_ablation_queue.ps1"
LAUNCHER_FILE="${TMP_DIR}/offline_ablation_queue_launcher.ps1"
REMOTE_QUEUE_PATH="${REMOTE_Codex_DIR}/offline_ablation_queue_${PREFIX}.ps1"
REMOTE_LAUNCHER_PATH="${REMOTE_Codex_DIR}/offline_ablation_queue_launcher_${PREFIX}.ps1"
...
function Test-RunProcessAlive(\$runStatus) {
  if (\$null -eq \$runStatus -or \$null -eq \$runStatus.pid) { return \$false }
  \$proc = Get-Process -Id ([int]\$runStatus.pid) -ErrorAction SilentlyContinue
  return \$null -ne \$proc
}

function Wait-ExistingRun(\$runId) {
  while (\$true) {
    \$status = Get-RunStatus \$runId
    if (\$null -eq \$status) { return 'missing' }
    if (\$status.status -eq 'succeeded') { return 'succeeded' }
    if (\$status.status -eq 'failed' -or \$status.status -eq 'stopped') { return \$status.status }
    if (!(Test-RunProcessAlive \$status)) { return 'not_running' }
    Write-QueueStatus 'waiting_existing' \$runId 'Waiting for already running background job.'
...
function Run-One(\$name, \$extraArgs) {
  \$runId = "\${prefix}_\${name}"
  \$existing = Wait-ExistingRun \$runId
  if (\$existing -eq 'succeeded') {
    Write-QueueStatus 'skipped_succeeded' \$runId 'Existing result found.'
    return
  }
  if (\$existing -eq 'failed') {
    Write-QueueStatus 'failed' \$runId 'Existing run failed; queue stops.'
    throw "Existing run failed: \$runId"
  }
...
try {
  ...
  Write-QueueStatus 'starting' '' 'Offline ablation queue starting.'

  Run-One 'lambda01' @('--fedfed_lambda_distill', '0.1')
  Run-One 'lambda03' @('--fedfed_lambda_distill', '0.3')
  Run-One 'warmup10' @('--fedfed_distill_warmup_rounds', '10')
  Run-One 'warmup20' @('--fedfed_distill_warmup_rounds', '20')
  Run-One 'no_anchor' @('--fedfed_enable_anchor', 'false')
  Run-One 'no_reliability' @('--fedfed_distill_count_tau', '0')
  Run-One 'no_ema' @('--fedfed_prototype_momentum', '0')
  Run-One 'no_feature_skew' @('--enable_feature_skew', 'false')

  Write-QueueStatus 'succeeded' '' 'Offline ablation queue finished.'
} catch {
  Add-Content -Path (Join-Path \$runsRoot ("\$prefix" + '_queue_error.log')) -Value (\$_ | Out-String)
  if ((Get-Content \$queueStatusPath -Raw | ConvertFrom-Json).status -ne 'failed') {
...
echo "Offline queue submitted: ${PREFIX}"
