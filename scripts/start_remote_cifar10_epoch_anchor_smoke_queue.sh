...
PREFIX="cifar10_epoch_anchor_smoke_$(date -u +%Y%m%dT%H%M%SZ)"
ROUND_NUM=100
BATCH_SIZE=256
...
QUEUE_FILE="${TMP_DIR}/epoch_anchor_smoke_queue.ps1"
LAUNCHER_FILE="${TMP_DIR}/epoch_anchor_smoke_queue_launcher.ps1"
REMOTE_QUEUE_PATH="${REMOTE_Codex_DIR}/epoch_anchor_smoke_queue_${PREFIX}.ps1"
REMOTE_LAUNCHER_PATH="${REMOTE_Codex_DIR}/epoch_anchor_smoke_queue_launcher_${PREFIX}.ps1"
...
try {
  ...
  Write-QueueStatus 'starting' '' 'CIFAR-10 epoch-scaled anchor smoke queue starting.'
  foreach (\$epoch in @('1', '5')) {
    Run-One 'fedavg' \$epoch 'none' @()
    Run-One 'epoch_scaled_anchor' \$epoch 'fedfed_prototype' @('--fedfed_distill_warmup_rounds', '10', '--fedfed_anchor_epoch_scaling', 'true', '--fedfed_lambda_anchor_max', '0.1', '--fedfed_anchor_ref_epoch', '5')
  }
  Write-QueueStatus 'succeeded' '' 'CIFAR-10 epoch-scaled anchor smoke queue finished.'
} catch {
...
echo "Offline epoch-scaled anchor smoke queue submitted: ${PREFIX}"
